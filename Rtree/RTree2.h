...
bool RTREE_QUAL::InsertRectRec(const Branch& a_branch, Node* a_node, Node** a_newNode, int a_level)
{
  ...
  if(a_node->m_level > a_level)
  {
    ...
    int index = PickBranch(&a_branch.m_rect, a_node);//!!!!!!!!!!!!!!!!!!找到适合的分支pickbranch 
...
RTREE_TEMPLATE
typename RTREE_QUAL::Rect RTREE_QUAL::NodeCover(Node* a_node)//^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^调整所有的孩子节点， 找到最小的覆盖范围 
{
  ASSERT(a_node);
...
int RTREE_QUAL::PickBranch(const Rect* a_rect, Node* a_node)
{
  ...
  for(int index=0; index < a_node->m_count; ++index)
  {
    ...
    tempRect = CombineRect(a_rect, curRect);//////!!!!!!!!!!!!!计算合并后增加的面积，选择增加面积最小的那个即为最优的序号  返回 
